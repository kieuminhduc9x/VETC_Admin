  ...
  <main-layout>
    ...
    <template v-slot:breadcrumb>
      <a-breadcrumb separator=">">
        ...
        <a-breadcrumb-item :class="'active'">Lịch trình bay</a-breadcrumb-item>
      </a-breadcrumb>
    ...
    <a-form-model
      ...
      <a-collapse v-model="activeSearchKey" expandIconPosition="left" class="collapse-left">
        <a-collapse-panel header="Tìm kiếm lịch trình bay" key="1">
          <a-card style="width: 100%;border: none" class="search-container">
            <a-row :gutter="16" type="flex" justify="center">
              ...
              <a-col :xs="24" :md="6" :lg="6" class="filter-item-container">
                <a-form-model-item prop="startDate" label="Từ ngày">
                  <a-date-picker
    ...
    <a-collapse v-model="activeResultKey" expandIconPosition="left" style=" margin-top: 8px" class="collapse-left">
      <a-collapse-panel header="Lịch trình bay" key="1">
          ...
          <a-row :gutter="16" type="flex">
            <a-col :span="24">
              ...
              <a-table
                ...
                <template slot="flightTime" slot-scope="text, record">
                  <span>{{ record.fromTime + ' - ' + record.toTime }} </span>
                </template>
...
import { FlightScheduleDelete, FlightScheduleSearch, FlightScheduleUpdateStatus } from '@/api/flight'
...
export default {
  ...
  data () {
    ...
    const validateSameProvince = (rule, value, callback) => {
      if (this.filters.fromProvince !== '' && this.filters.fromProvince !== '' && this.filters.fromProvince && this.filters.fromProvince === this.filters.toProvince) {
        callback(new Error('Từ Tỉnh/TP và Đến Tỉnh/TP không được phép trùng'))
      } else {
        this.$refs.ruleForm.validateField('toProvince')
        callback()
      }
    }
    return {
      ...
      rules: {
        toProvince: [
          {
            validator: validateSameProvince,
            trigger: 'change'
          }
        ]
      },
  ...
  methods: {
    ...
    onChangeStartDate (value, dateString) {
      this.filters.startDate = value ? value.format('YYYY-MM-DD') : null
    },
    onChangeEndDate (value, dateString) {
      this.filters.endDate = value ? value.format('YYYY-MM-DD') : null
    },
    ...
    getData () {
      ...
      this.loading = true
      this.data = []
      FlightScheduleSearch(_merge(params, this.filters)).then(res => {
    ...
    addNew () {
      this.modalTitle = 'Thêm mới Lịch trình bay'
    ...
    onEditRow (record) {
      ...
      this.modalTitle = 'Cập nhật Lịch trình bay'
    ...
    onDeleteRow (record, index) {
      this.$confirm({
        title: 'Bạn có muốn xóa Lịch trình bay này?',
        okText: 'Có',
        okType: 'primary',
        cancelText: 'Không',
        onOk: () => {
          this.deleteRow(record.flightScheduleId)
        },
        onCancel () {
        }
      })
    ...
    deleteRow (id) {
      const $this = this
      this.loading = true
      FlightScheduleDelete({ flightScheduleId: id })
        .then(rs => {
          this.$notification.success({
            message: 'Xóa Lịch trình bay',
            description:
            'Xóa Lịch trình bay thành công',
            duration: 5
          })
          $this.getData()
        })
    ...
    changeStatus (record) {
      this.loading = true
      FlightScheduleUpdateStatus({ flightScheduleId: record.flightScheduleId, status: record.status ? 0 : 1 })
        .then(rs => {
