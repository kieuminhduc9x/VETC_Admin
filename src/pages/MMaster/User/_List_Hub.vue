<template>
  <div>
    ...
    <a-modal
      ...
      title="Thêm HUB làm việc"
      @cancel="handleClosePopup"
      width="80%"
      centered
      :footer="null"
    >
      <ListAddHub :user-object="userObject" v-if="userObject"></ListAddHub>
    </a-modal>

  </div>
</template>
<script>
import columns from './_hub_columns'
import {
  GetHubForUser, RemoveUserInHub
} from '@/api/user'
import resizeableTitle from '@/utils/resizable-columns'
import moment from 'moment'
import ListAddHub from './_Add_Hub'
...
export default {
  ...
  methods: {
    ...
    removeUser (record) {
      this.loading = true
      RemoveUserInHub({ hubId: record.hubId, userId: this.userObject.userId }).then(res => {
    ...
    fetchData () {
      this.loading = true
      GetHubForUser({ userId: this.userObject.userId }).then(res => {
        this.data = res
      }).catch(err => {
        const msg = this.handleApiError(err)
        this.$notification.error({
