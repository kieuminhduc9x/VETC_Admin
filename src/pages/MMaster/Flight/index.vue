  ...
  <main-layout>
    ...
    <template v-slot:breadcrumb>
      <a-breadcrumb separator=">">
        ...
        <a-breadcrumb-item :class="'active'">Danh mục chuyến bay</a-breadcrumb-item>
      </a-breadcrumb>
    ...
    <a-collapse v-model="activeResultKey" expandIconPosition="left" style=" margin-top: 8px" class="collapse-left">
      <a-collapse-panel header="Danh mục chuyến bay" key="1">
...
import { FlightSearch, FlightDelete } from '@/api/flight'
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      this.loading = true
      this.data = []
      FlightSearch(_merge(params, this.filters)).then(res => {
    ...
    addNew () {
      this.modalTitle = 'Thêm mới chuyến bay'
    ...
    onEditRow (record) {
      ...
      this.modalTitle = 'Cập nhật chuyến bay'
    ...
    onDeleteRow (record, index) {
      this.$confirm({
        ...
        cancelText: 'Không',
        onOk: () => {
          this.deleteRow(record.flightId)
        },
        onCancel () {
        }
      })
    ...
    deleteRow (id) {
      const $this = this
      this.loading = true
      FlightDelete({ flightId: id })
        .then(rs => {
          this.$notification.success({
            message: 'Xóa chuyến bay',
            description:
