...
import { acceptExportImportExportManagement } from '@/api/import-export-management'
...
export default {
  ...
  mounted () {
    if (this.visibleAcceptExport === true) {
      this.visible = true
    } else {
      this.visible = false
    }
  ...
  methods: {
    ...
    submitConfirm () {
      ...
      this.loading = true
      acceptExportImportExportManagement(params).then(rs => {
        if (rs) {
