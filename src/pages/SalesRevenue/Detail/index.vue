  ...
  <main-layout>
    ...
    <template v-slot:breadcrumb>
      <a-breadcrumb separator=">">
        ...
        <a-breadcrumb-item :class="'active'">Báo cáo doanh thu nhà bán hàng</a-breadcrumb-item>
      </a-breadcrumb>
    ...
    <a-form-model
      ...
      <a-collapse v-model="activeSearchKey" expandIconPosition="left" class="collapse-left">
        <a-collapse-panel header="Tìm kiếm báo cáo doanh thu nhà bán hàng" key="1">
          <a-card style="width: 100%;border: none" class="search-container">
            <a-row :gutter="16" type="flex" justify="center">
              <a-col :xs="24" :md="6" :lg="6" class="filter-item-container">
                <a-form-model-item label="Nhà bán hàng" prop="shopCode">
                  <a-select
                    style="width: 100%"
                  ...
                  >
                    ...
                    <a-select-option v-for="item in listSupplierCode" :key="item.hrvSupplierId" :value="item.code">
                      {{ item.name }}
                    </a-select-option>
                  </a-select>
                </a-form-model-item>
...
import { searchSalesRevenueDetail } from '@/api/sales_revenue'
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      this.loading = false
      this.data = []
      searchSalesRevenueDetail(_merge(params, this.filters)).then(res => {
      ...
      this.$refs.ruleFilter.validate(valid => {
        if (valid) {
          const params = {
            ...
            reportName: 'report_revenue_by_supplier'
          }
          this.loadingExport = true
          exportReport(params)
            .then(rs => {
              const date = new Date(moment())
              const fileName = 'Bao_cao_chi_tiet_doanh_thu_nha_ban_hang_' + moment(date, 'YYYY_MM_DD') + '.' + type
