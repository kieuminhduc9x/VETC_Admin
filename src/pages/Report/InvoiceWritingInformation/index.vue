  ...
  <main-layout>
    ...
    <template v-slot:breadcrumb>
      <a-breadcrumb separator=">">
        ...
        <a-breadcrumb-item :class="'active'">Thông tin viết hóa đơn</a-breadcrumb-item>
      </a-breadcrumb>
    ...
    <a-form-model
      ...
      <a-collapse v-model="activeSearchKey" expandIconPosition="left" class="collapse-left">
        <a-collapse-panel header="Tìm kiếm thông tin viết hóa đơn" key="1">
...
export default {
  ...
  methods: {
    ...
    onChangeToDate (value, dateString) {
      this.filters.toDate = value ? value.format('YYYY-MM-DD') : null
    },
    resetForm (e) {
      ...
      this.$refs.ruleFilter.validate(valid => {
        if (valid) {
          const params = {
            ...
            reportName: 'report_sales_revenue'
          }
          this.loadingExport = true
          exportReport(params)
            .then(rs => {
              const date = new Date(moment())
              const fileName = 'Bao_cao_doanh_thu_nha_ban_hang_' + moment(date, 'YYYY_MM_DD') + '.' + type
