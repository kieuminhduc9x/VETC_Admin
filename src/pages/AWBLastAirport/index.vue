  ...
  <main-layout>
    ...
    <template v-slot:breadcrumb>
      <a-breadcrumb separator=">">
        ...
        <a-breadcrumb-item :class="'active'">Nhận AWB tại sân bay cuối</a-breadcrumb-item>
      </a-breadcrumb>
...
import {
  ReceiveAtLastAirport,
  SearchOrderMergeToReceiveAtLastAirport
} from '@/api/order'
...
export default {
  ...
  methods: {
    ...
    confirmReceipt (record) {
      ReceiveAtLastAirport({ orderMergeId: record.orderMergeId })
        .then(rs => {
          this.$notification.success({
            message: 'Nhận AWB tại sân bay cuối',
            description:
              'Nhận AWB tại sân bay cuối thành công',
            duration: 5
          })
          this.getData()
        })
    ...
    getData () {
      ...
      this.loading = true
      this.data = []
      SearchOrderMergeToReceiveAtLastAirport(_merge(params, this.filters)).then(res => {
